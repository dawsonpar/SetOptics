...
export function CollectionsPanel() {
  ...
  // whole panel collapsed on small screens by default to free the top area
  ...
  return (
    <div
      className={`glass flex max-h-[64vh] flex-col p-2.5 ${
        panelOpen ? 'w-[min(16rem,calc(100vw-1.5rem))]' : 'w-auto'
      }`}
    >
      <div className="flex items-center justify-between gap-2">
        ...
        >
          ...
          {collections.length > 0 && (
            <span className="font-mono text-[11px] text-muted">· {collections.length}</span>
          )}
        </button>
        {panelOpen && (
